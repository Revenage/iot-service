...
difference()
{
	...
	union()
	{
		...
		//Left mounting lug
		color("SILVER") translate([-35/2,0,18.5 - diff]) mountingLug(diff);
		
		//Right mounting lug
		color("SILVER") translate([35/2,0,18.5 - diff]) rotate([0,0,180]) mountingLug(diff);
...
module mountingLug(diff)
{
	...
	difference()
	{
		hull()
		{
			cylinder(r=7/2, h=0.5 + diff, $fn=40);
			translate([0,-7/2,0]) cube([7,7,0.5 + diff]);
		}
		if (diff == 0)
